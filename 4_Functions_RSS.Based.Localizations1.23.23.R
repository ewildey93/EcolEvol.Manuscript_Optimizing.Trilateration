...
beep.dat <- beep.dat[beep.dat$TagId %in% test.info$TagId,]
...
dt1 <- data.table(beep.dat, start=beep.dat$Time.local, end=beep.dat$Time.local)
dt2 <- data.table(test.info)
setkey(dt2, TagId, start, end)
test.dat <- foverlaps(dt1, dt2, type='within',nomatch = NULL)
#test.dat <- beep.dat %>%
